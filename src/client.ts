...
type FileType = '-'
type LinkType = 'l'
type FileTypes = DirectoryType | FileType | LinkType
...
export class SFTPClient {
    ...
    async exists(path: string): Promise<boolean | FileTypes> {
        if (!this.sftp) {
            throw new Error('No SFTP connection available')
        }
        const readdir = promisify(this.sftp.readdir.bind(this.sftp))
    ...
    async listAndCleanEmptyFiles(path: string, filter = /.*/) {
        ...
        const files = all.filter(content => content.type === '-' && content.size !== 0)
        const emptyFiles = all.filter(content => content.type === '-' && content.size === 0)
